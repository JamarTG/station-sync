...
export function ChargesAttendantDetailModal({ attendant, entries, onBack, onClose }: Props) {
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-[36px] font-bold text-[#111] leading-none">{attendant}</h2>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {entries.map((e, i) => (
            <div key={i} className="flex items-start justify-between py-2">
              <div>
                <p className="text-[13px] font-semibold text-[#111]">{e.collectedBy}</p>
                <p className="text-[11px] font-medium text-[#888] mt-0.5">{e.litres.toFixed(2)} L</p>
              </div>
        ...
        <div className="flex items-center justify-between pt-4">
          ...
          <p className="text-[13px] font-semibold text-[#333]">{entries.length} Items</p>
        </div>
      </div>
    </div>
  )
